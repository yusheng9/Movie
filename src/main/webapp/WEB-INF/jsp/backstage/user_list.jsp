...
<div class="page-container">
	...
	<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"><a href="javascript:;" onclick="delAll()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a> <a href="javascript:;" onclick="member_add('添加用户','${pageContext.request.contextPath }/user/useradd','','510')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加用户</a></span> </div>
	<div class="mt-20">
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		<tbody>
			<c:forEach items="${list }" var="li">
				<tr class="text-c">
					<td><input type="checkbox" class="checkboxs" value="${li.userId }" name=""></td>
					<td>${li.userId }</td>
					<td><u style="cursor:pointer" class="text-primary" onclick="member_show('详细信息','${pageContext.request.contextPath }/user/usershow/${li.userId }','${li.userId }','410','480')">${li.userName }</u></td>
					<td>${li.userSex }</td>
					<td>${li.userPhone }</td>
					<td>${li.userEmail }</td>
					<td class="text-l">${li.province } ${li.city } ${li.district }</td>
					<td>${li.createTime }</td>
					<c:if test="${li.userStatus==1 }">
						<td>
							<div style="width: 73px;" class="switchs" id="switch${li.userId}" onclick="switchs('${li.userId}');">
						        <input type="checkbox" name="check-3" value="${li.userId}" class="lcs_check lcs_tt1" title="启用" checked="checked"/>
						    </div>
					    </td>
					</c:if>
					<c:if test="${li.userStatus==2 }">
						<td class="td-status">
							<div style="width: 73px;" class="switchs" id="switch${li.userId}" onclick="switchs('${li.userId}');">
						        <input type="checkbox" name="check-3" value="${li.userId}" class="lcs_check lcs_tt1" title="停用"/>
						    </div>
					    </td>
					</c:if>
					<td class="td-manage"> <a title="编辑" href="javascript:;" onclick="member_edit('编辑','${pageContext.request.contextPath }/user/usermodify/${li.userId }','4','','510')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a> <a style="text-decoration:none" class="ml-5" onClick="change_password('修改密码','${pageContext.request.contextPath }/user/changePassword/${li.userId}','10001','600','270')" href="javascript:;" title="修改密码"><i class="Hui-iconfont">&#xe63f;</i></a> <a title="删除" href="javascript:;" onclick="member_del(this,'${li.userId}')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a></td>
				</tr>
			</c:forEach>
...
$(function(){
	...
	$("#button").click(function() {
		location.href = "${pageContext.request.contextPath }/user/selectUserLike?createTime="+$("#datemin").val()+"&createTime2="+$("#datemax").val()+"&user="+$("#user").val()+"";
		/* $.ajax({
			type: "GET",//传输方式
    ...
    layer.confirm('确认要删除吗？'+ids.toString(),function(){
        $.ajax({ 
        	...
        	success: function(data) { 
        		...
        		//$(".checkboxs").not('.header').parents('tr').remove();
        		setTimeout(function(){
  ...
  function switchs(id){
	...
	if(title=="启用"){
		layer.confirm('确认要停用吗？',function(index){
			$.ajax({ 
	        	type: "POST", 
	        	url: '${pageContext.request.contextPath }/user/blockup/'+id+'',
	        	success: function(data) { 
		...
		layer.confirm('确认要启用吗？',function(index){
			$.ajax({ 
	        	type: "POST", 
	        	url: '${pageContext.request.contextPath }/user/startusing/'+id+'',
	        	success: function(data) { 
